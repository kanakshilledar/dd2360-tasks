...
int main(int argc, char **argv) {
  ...
  cusparseDnVecDescr_t vecTemp;     // Dense vector descriptor for temp
  cusparseDnVecDescr_t vecTmp;      // Dense vector descriptor for tmp
  ...
  if (concurrentAccessQ) {
    ...
    cpuLocation.type = cudaMemLocationTypeHost;
    cpuLocation.id = 0;
    gpuCheck(cudaMemPrefetchAsync(temp, sizeof(double) * dimX, cpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(tmp, sizeof(double) * dimX, cpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(A, sizeof(double) * nzv, cpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(ARowPtr, sizeof(int) * (dimX + 1), cpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(AColIndx, sizeof(int) * nzv, cpuLocation, 0, 0));
  ...
  if (concurrentAccessQ) {
    ...
    gpuCheck(cudaMemPrefetchAsync(temp, sizeof(double) * dimX, gpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(tmp, sizeof(double) * dimX, gpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(A, sizeof(double) * nzv, gpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(ARowPtr, sizeof(int) * (dimX + 1), gpuLocation, 0, 0));
    gpuCheck(cudaMemPrefetchAsync(AColIndx, sizeof(int) * nzv, gpuLocation, 0, 0));
  ...
  //@@ Insert code to set the cuBLAS pointer mode to CUBLAS_POINTER_MODE_HOST
  ...
  cusparseCheck(cusparseCreateCsr(&matA, dimX, dimX, nzv,
      ARowPtr, AColIndx, A,
      CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
      CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&vecTemp, dimX, temp, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&vecTmp, dimX, tmp, CUDA_R_64F));
  ...
  cusparseCheck(cusparseDestroyDnVec(vecTemp));
  cusparseCheck(cusparseDestroyDnVec(vecTmp));
